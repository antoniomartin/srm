  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
      {cards.map((card, idx) => (
        <div 
          key={idx} 
          className={`flex items-center gap-3 p-4 rounded-xl border ${card.bg} transition-all duration-200 hover:shadow-sm`}
        >
          <div className="p-2 bg-white rounded-lg border border-slate-100 shadow-sm flex-shrink-0">
            {card.icon}
          </div>
          <div className="min-w-0">
            <p className="text-xs font-semibold text-slate-500 line-clamp-1 uppercase tracking-wider">{card.title}</p>
